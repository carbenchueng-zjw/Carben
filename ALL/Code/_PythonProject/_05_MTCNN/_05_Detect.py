...
class Detector():
    ...
    def __init__(self, pnet_param="./parameter/pnet.pt：0.01902.pt",
                 ...
                 onet_param="./parameter/onet.pt：0.00460.pt"):
    ...
    def __pnet_detect(self, image):  # ★p网络全部是卷积，与输入图片大小无关，可输出任意形状图片
        ...
        while min_side_len > 12:  # 直到缩放到小于等于12时停止
            ...
            img_data = img_data.to(Device)  # 将图片tensor传到cuda里加速
            ...
            # print("_cls",_cls[0][0].shape)         # [1, 1, 203, 245]:NCWH：分组卷积的特征图的通道和尺寸★
    ...
    def __rnet_detect(self, image, pnet_boxes):
        ...
        _pnet_boxes = ConvertToSquare(pnet_boxes)  # ★给p网络输出的框，找出中心点，沿着最大边长的两边扩充成“正方形”，再抠图
        # print(type(_pnet_boxes))
        ...
        img_dataset = img_dataset.to(Device)  # 给图片数据采用cuda加速
    ...
    def __onet_detect(self, image, rnet_boxes):
        ...
        _rnet_boxes = ConvertToSquare(rnet_boxes)  # 给r网络输出的框，找出中心点，沿着最大边长的两边扩充成“正方形”
        ...
        return Nms(np.array(boxes), o_nms)  # 用最小面积的IOU；原o_nms(IOU)为小于0.7的框被保留下来
    ...
    def detect(self, image):  # 检测图片
        ...
        if pnet_boxes[0].shape == 0:  # 若P网络没有人脸时，避免数据出错，返回一个新数组
            return np.array([])
        ...
        if onet_boxes[0].shape == 0:  # 若P网络没有人脸时，避免数据出错，返回一个新数组
            return np.array([])
        end_time = time.time()
...
if __name__ == '__main__':
    image_path = r"D:\1-Git\Code\_PythonProject\_05_MTCNN\test_images\01.jpg"
    detector = Detector()
